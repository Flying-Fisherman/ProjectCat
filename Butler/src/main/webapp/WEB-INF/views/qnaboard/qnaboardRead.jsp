...
<body>
	...
	<script>
		...
		function update1()
		{
			if(confirm("修正しますか。"))
			{
				location.href = "<c:url value = '/qnaboard/qnaboardUpdate?boardnum=${requestScope.read.boardnum}'/>";
			}
		}
		...
		function replyDelete(a,b)
		{
			if(confirm("削除しますか。"))
			{
				location.href = "<c:url value ='/qnaboard/replyDelete?replynum='/>"+a+"&boardnum="+b;
		...
		function backmenu()
		{
			location.href = "<c:url value = '/qnaboard/qnaboardList'/>"
		}
...
<table frame=void style="margin-top: 70px;margin-left: auto; margin-right: auto;" border= "1">
	<tr>
...
<!-- 			수정 / 삭제 / 버튼은 게시글 작성자와 접속 유저가 동일할 경우에만 출력-->
			<c:if test = "${read.userid == sessionScope.userid}">
				...
				<a href = "<c:url value = '/qnaboard/delete?boardnum=${read.boardnum}'/>">
	...
	<tr>
		...
		<td id = "boardfile" class="board-td">
			<a href = "<c:url value = '/qnaboard/download?boardnum=${read.boardnum}'/>">
	...
	<form action="<c:url value = '/qnaboard/replyWrite'/>" method = "post">
			...
			<table id = "replyinput" style="margin-left: auto; margin-right: auto;">
				<tr>
					<td>
						...
						<input id = "replysubmit" type = "submit" value = "コメント入力"/>
					</td>
				</tr>
			</table>
		</form>
...
<div id = "replydisplay" style="margin-bottom: 50px;">
	<table class = "reply" style="margin-left: auto; margin-right: auto; border-color: black; background-color: white; " border="1">
		...
		<tr>
			<th style="color: black; width: 350px; text-align: center; background-color: #E6A756;">内容</th>
			<th style="color: black; width: 70px; text-align: center; background-color: #E6A756;">投稿者</th>
			<th colspan="2" style="color: black; width: 180px; text-align: center; background-color: #E6A756;">投稿日</th>
		</tr>
    ...
    <div class="container">
      <p class="m-0 small">Copyright &copy; 집사의 조건</p>
    </div>
  </footer>
